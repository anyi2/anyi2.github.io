...
if ($the_host !== 'bbs.dananjie.com') //验证当前访问域名（就是你的新域名）：若非引号内的域名，则进行如下跳转——
{
if ($querystring !== '') //验证文件名后是否有参数，如果有参数则跳转到——
{
header('HTTP/1.1 301 Moved Permanently'); //发出301头部，表明永久重定向
header('Location: http://bbs.dananjie.com/'.$filename.'?'.$querystring); //跳转到我的新域名地址【带参数】
}
elseif ($filename == 'index.php') //如果是主页则直接跳转到新域名
{
header('HTTP/1.1 301 Moved Permanently'); //发出301头部，表明永久重定向
header('Location: http://bbs.dananjie.com/'); //跳转到我的新域名
}
else //如果无参数则跳转到——
{
header('HTTP/1.1 301 Moved Permanently'); //发出301头部，表明永久重定向
header('Location: http://bbs.dananjie.com/'.$filename); //跳转到我的新域名地址【不带参数】
}
}
?>
